...
threshold && sensorValues[3] > threshold && sensorValues[4] > threshold && sensorValues[5] >
threshold && sensorValues[6] > threshold)
{
digitalHigh( rightMotor2 );
digitalHigh( rightMotor1 );
digitalHigh( leftMotor2 );
...
if (leftMotorSpeed > MaxSpeed ) leftMotorSpeed = MaxSpeed; // prevent the motor from
going beyond max speed
...
line_position = qtrrc.readLine(sensorValues);
while (sensorValues[2] > threshold || sensorValues[3] > threshold) // wait for outer most
sensor to find the line
{
line_position = qtrrc.readLine(sensorValues);
}
digitalLow( leftMotor1 );
...
line_position = qtrrc.readLine(sensorValues);
while (sensorValues[4] > threshold || sensorValues[3] > threshold) // wait for outer most
sensor to find the line
{
line_position = qtrrc.readLine(sensorValues);
}
...
delay(140);
qtrrc.readLine(sensorValues);
if (sensorValues[2] > threshold && sensorValues[3] > threshold && sensorValues[4] >
threshold)
{ digitalLow( rightMotor2 );
digitalHigh( rightMotor1 );
digitalLow( leftMotor2 );
...
analogWrite(rightMotorPWM, 100);
analogWrite(leftMotorPWM, 100);
delay(100); //TIME BEFORE TAKING RIGHT TURN DURING ACTUAL
RUN
}
else {
digitalLow( rightMotor2 );
