...
virtual void read(unsigned int *sensor_values, unsigned char readMode =
QTR_EMITTERS_ON);
...
void readCalibrated(unsigned int *sensor_values, unsigned char readMode =
QTR_EMITTERS_ON);
...
virtual void readPrivate(unsigned int *sensor_values, unsigned char step = 1, unsigned char
start = 0) = 0;
unsigned char *_pins;
...
unsigned int timeout = 2500, unsigned char emitterPin =
...
// sensors.
void readPrivate(unsigned int *sensor_values, unsigned char step = 1, unsigned char start =
0) override;
};
...
calmin = calibratedMinimumOn[i] + _maxValue - calibratedMinimumOff[i]; // this won't go
past _maxValue
if (calibratedMaximumOff[i] < calibratedMaximumOn[i]) // no meaningful signal
calmax = _maxValue;
else
calmax = calibratedMaximumOn[i] + _maxValue - calibratedMaximumOff[i]; // this won't go
past _maxValue
}
...
{
if (readMode == QTR_EMITTERS_ODD_EVEN || readMode ==
...
if (readMode != QTR_EMITTERS_MANUAL)
emittersOff();
if (readMode == QTR_EMITTERS_ON_AND_OFF || readMode ==
QTR_EMITTERS_ODD_EVEN_AND_OFF)
{
unsigned char i;
unsigned int off_values[QTR_MAX_SENSORS];
...
// sensor). For example, step = 2, start = 1 means read the *even-numbered*
// sensors.
void QTRSensorsRC::readPrivate(unsigned int *sensor_values, unsigned char step, unsigned
char start)
{
unsigned char i;
if (_pins == 0)
...
// sensor). For example, step = 2, start = 1 means read the *even-numbered*
// sensors.
void QTRSensorsAnalog::readPrivate(unsigned int *sensor_values, unsigned char step,
unsigned char start)
{
unsigned char i, j;
if (_pins == 0)
...
#define Kd 2
#define Ki .01
#define MaxSpeed 255 //change speed here
#define BaseSpeed 255 //change speed here keep base speed and max speed same
#define NUM_SENSORS 7
#define TIMEOUT 2500
#define EMITTER_PIN QTR_NO_EMITTER_PIN
#define speedturn 200 // edit this to determine the speed during taking turns
#define rightMotor1 A3
#define rightMotor2 A4
#define rightMotorPWM 9
#define leftMotor1 A1
#define leftMotor2 A0
#define leftMotorPWM 10
#define motorPower A2
#define threshold 700
#define led 13
#define delaybeforeleftturn 50
...
void setup()
{ TCCR1B = TCCR1B & B11111000 | B00000001; //creates 31khz pwm frequency at pin 9,10
...
for (int i = 0; i <= 200; i++) // duration of calibration cycle
{
...
qtrrc.calibrate(); // reads all sensors with the define set 2500 microseconds (25
milliseconds) for sensor outputs to go low.
}
...
void loop()
{
follow_line_black_on_white();
}
void follow_line_white_on_black()
{while(1)
{ line_position = qtrrc.readLine(sensorValues);
if (sensorValues[0] < threshold && sensorValues[6] < threshold) //if both outer most sensor
has value lower than threshold that means we are on white surface with black line and call the
...
if (rightMotorSpeed > MaxSpeed ) rightMotorSpeed = MaxSpeed; // prevent the motor from
going beyond max speed
if (leftMotorSpeed > MaxSpeed ) leftMotorSpeed = MaxSpeed; // prevent the motor from going
beyond max speed
...
void follow_line_black_on_white()
{while(1)
{line_position = qtrrc.readLine(sensorValues, QTR_EMITTERS_ON, 1);
if (sensorValues[0] > threshold && sensorValues[6] > threshold) //if both outer most sensor
has value higher than threshold that means we are on black surface with white line and call the
respective function
{
follow_line_white_on_black();
}
int error = line_position - 3000;
int error1 = error - lastError;
int error2 = (2.0 / 3.0) * error2 + error ;
